...
public class TodoListImplTest {
	...
	public void filterByPriority() {
		...
		assertEquals(2, todoList
						.filter(Criteria.priority(Priority.NORMAL))
						.getTasks().size());
	}
	...
	public void testJoin() {
		...
		assertEquals(4, todoList
						.filter(Criteria.priority(Priority.LOW))
						.join(todoList
								.filter(Criteria.status(Status.DOING)))
						.getTasks().size());
		...
		assertEquals(3, todoList
                .filter(Criteria.priority(Priority.LOW))
                .join(todoList.filter(Criteria.tags(new String[] { "school" }))).getTasks().size());
	}
	...
	public void filterByNotCriteria() {
		...
		assertEquals(3, todoList
						.filter(Criteria.priority(Priority.LOW)
								.not()
								.or(Criteria.tags(new String[] { "school" })))
						.getTasks().size());
	}	
