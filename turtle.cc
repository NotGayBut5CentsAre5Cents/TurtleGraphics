...
Turtle& SVGTurtle::moveto(const Point& next_pos) {
    ...
    if(is_pendown()) {
        ...
        out_ << "\t style=\"stroke:rgb(" << get_color().red() << ","
             << get_color().green() << "," << get_color().blue() << ");"
...
PolygonDrawer::PolygonDrawer() :
        number_of_sides_(3), format_("svg"), type_("STAR"),
        width_of_canvas_(1000), height_of_canvas_(1000), out_(cout) {
            turtle_ = new SVGTurtle(width_of_canvas_, height_of_canvas_, out_);
        }
PolygonDrawer::PolygonDrawer(int number_of_sides, string format, string type, double width, double height, ostream& out) :
        number_of_sides_(number_of_sides > 2 ? number_of_sides : 3),
        format_(format), type_(type),
        width_of_canvas_(width), height_of_canvas_(width), out_(out) {
            assert(format == "svg" || format == "esp");
            turtle_ = format == "svg"
                    ? (Turtle*) new SVGTurtle(width_of_canvas_, height_of_canvas_, out_)
                    : (Turtle*) new PSTurtle(width_of_canvas_, height_of_canvas_, out_);
        }

void PolygonDrawer::calculate_internal_angle() {
    assert(number_of_sides_ > 2);
    internal_angle_ = 180 * (number_of_sides_ - 2) / number_of_sides_;
}
...
PolygonDrawer& PolygonDrawer::set_num_of_sides(int num_of_sides) {
    ...
    number_of_sides_ = num_of_sides;
    this->calculate_external_angle();
    this->calculate_internal_angle();
    return *this;
}
